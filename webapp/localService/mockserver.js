    ...
    function (JSONModel, sinon, Log) {
        ...
        function readData() {
            var oMetadataPromise = new Promise(function (fnResolve, fnReject) {
                ...
                oRequest.onerror = function () {
                    var sError =
                        "error loading resource '" + sResourcePath + "'";
                    Log.error(sError, sLogComponent);
                    fnReject(new Error(sError, sLogComponent));
                };
            ...
            var oMockDataPromise = new Promise(function (fnResolve, fnReject) {
                ...
                oMockDataModel.attachRequestCompleted(function (oEvent) {
                    ...
                    if (
                        ...
                        var sError =
                            "resource '" + sResourcePath + "' not found";
                        Log.error(sError, sLogComponent);
                        fnReject(new Error(sError, sLogComponent));
                    }
        ...
        function handleGetUserRequests(oXhr, bCount) {
            ...
            if (sKey) {
                ...
                if (/People\(.+\)\/Friends/.test(oXhr.url)) {
                    ...
                    oResponse = { value: [] };
                    oResponse.value = createFriendsArray(
                        aUsers[iIndex].Friends,
                        aSelect
                    );
                } else {
                    // specific user was requested
                    oResponse = getUserObject(
                        iIndex,
                        aSelect,
                        aExpand,
                        aSubSelects
                ...
                for (var i = 0; i < aExpand.length; i++) {
                    switch (aExpand[i]) {
                        case "Friends":
                            ...
                            oObject.Friends = createFriendsArray(
                                aFriends,
                                aSubSelects[i]
                            );
                            break;
                        case "BestFriend":
                            ...
                            oObject.BestFriend = getUserByIndex(
                                iFriendIndex,
                                aSubSelects[i]
                            );
                            break;
        ...
        function handleBatchRequest(oXhr) {
            var aResponse,
                ...
                sInnerBoundary,
                sPartBoundary,
                aOuterParts = oXhr.requestBody
                    .split(sOuterBoundary)
                    .slice(1, -1), // The individual requests
                aParts,
                aMatches;
            ...
            if (aMatches && aMatches.length > 0) {
                ...
                aParts = aOuterParts[0]
                    .split("--" + sInnerBoundary)
                    .slice(1, -1);
            } else {
                aParts = aOuterParts;
            }
